...
const ShowEmailAddress = ({
  ...
  const handleShowEmailAddress = async () => {
    ...
    try {
      ...
      const emailAddress =
        emailType === "accounts"
          ? (await fetchAccountsEmail(recaptchaToken)) || "Email not found"
          : (await fetchDispensaryEmail(recaptchaToken)) || "Email not found";
  ...
  if (showEmail === "Show email address") {
    ...
    return (
      <Link
        ...
        className={classNames(
          "py-2 text-left px-2 -my-2 -mx-2 text-paragraph self-start tablet:hover:opacity-80 desktop:p-0 desktop:m-0",
          linkClasses
        )}
      >
        {showEmail}
